...
export const api = {
  ...
  renameLocation: (id: string, label: string) => request<Location>(`/locations/${id}`, { method: 'PATCH', body: JSON.stringify({ label }) }),
  defaultLocation: (id: string) => request<Location>(`/locations/${id}/default`, { method: 'POST' }),
  deleteLocation: (id: string) => request<{ status: string }>(`/locations/${id}`, { method: 'DELETE' }),
