...
export function AutoSignals({ group, groups, intents, locations, onChanged }: { group: Group; groups: Group[]; intents: Intent[]; locations: Location[]; onChanged: () => void }) {
  ...
  return <section className="page-stack"><SectionHeader title="Автосигналы" action={<button type="button" className="icon-button" aria-label="Новый автосигнал" onClick={() => setCreating(true)}><Icon name="plus" /></button>} />{error ? <p className="form-error" role="alert">{error}</p> : null}
    {autos.length ? <div className="autos-list">{autos.map(intent => <article className="autosignal-row" key={intent.id}>
      <button type="button" className="autosignal-row__main" onClick={() => setEditing(intent)} aria-label={`Изменить ${intent.name || 'автосигнал'}`}>
        <strong>{intent.name || 'Мой ДВИЖ'}</strong>
        <span>{weekDays(intent.weekdays)}{intent.local_start && intent.local_end ? ` · ${intent.local_start}–${intent.local_end}` : ''}</span>
        <span>{(intent.activity_categories || [intent.activity_category]).map(activityLabel).join(', ')} · {intent.group_name || groups.find(item => item.id === intent.group_id)?.name}</span>
        {intent.budget_max !== null || intent.radius_km !== null ? <span>{intent.budget_max !== null ? `до ${intent.budget_max} ₽` : null}{intent.budget_max !== null && intent.radius_km !== null ? ' · ' : null}{intent.radius_km !== null ? `${intent.radius_km} км` : null}</span> : null}
      </button>
      <label className="switch-control"><span className="sr-only">{intent.status === 'ACTIVE' ? 'Поставить на паузу' : 'Возобновить'} {intent.name}</span><Switch checked={intent.status === 'ACTIVE'} onChange={() => void toggle(intent)} /></label>
      <button type="button" className="text-action text-action--danger autosignal-row__delete" onClick={() => void remove(intent)}>Удалить</button>
    </article>)}</div> : <EmptyState title="Автосигналов пока нет">Создай правило — приглашение придёт, когда условия совпадут.</EmptyState>}
    {!autos.length ? <Button className="new-auto" variant="primary" onClick={() => setCreating(true)}>Создать автосигнал</Button> : null}
  </section>
}
...
function AutoSignalForm({ group, groups, editing, locations, onDone, onCancel }: { group: Group; groups: Group[]; editing: Intent | null; locations: Location[]; onDone: () => void; onCancel: () => void }) {
  ...
  return <section className="auto-form"><button className="back-link" onClick={onCancel}>← Назад</button><h1>{editing ? 'Автосигнал' : 'Новый автосигнал'}</h1><form onSubmit={submit}>
    ...
    <div className="form-section"><h2>Что ок</h2><div className="choices">{['any', 'games', 'sport', 'exhibition', 'concert', 'wellness'].map(category => <button type="button" key={category} className={`choice ${selectedCategories.includes(category) ? 'choice--selected' : ''}`} aria-pressed={selectedCategories.includes(category)} onClick={() => toggleCategory(category)}>{activityLabel(category)}</button>)}</div></div>
    <div className="form-section"><h2>Расписание</h2><div className="choices choices--weekdays">{weekdays.map(day => <button key={day.value} type="button" className={`choice ${selectedWeekdays.includes(day.value) ? 'choice--selected' : ''}`} aria-pressed={selectedWeekdays.includes(day.value)} onClick={() => toggleWeekday(day.value)}>{day.label}</button>)}</div></div>
    ...
    <div className="form-section"><h2>Условия</h2><div className="form-row"><label>Бюджет, ₽ <Input aria-label="Бюджет" type="number" min="0" max="100000" step="1" placeholder="Неважно" value={budget} onChange={event => setBudget(event.target.value)} /></label><label>Радиус, км <Input aria-label="Радиус" type="number" min="0.1" max="100" step="0.1" placeholder="Неважно" value={radius} onChange={event => setRadius(event.target.value)} /></label></div>
    <h3>Сколько человек</h3><div className="choices"><button type="button" className={`choice ${people === 'any' ? 'choice--selected' : ''}`} aria-pressed={people === 'any'} onClick={() => setPeople('any')}>Неважно</button><button type="button" className={`choice ${people === '3+' ? 'choice--selected' : ''}`} aria-pressed={people === '3+'} onClick={() => setPeople('3+')}>Хотя бы 3</button><button type="button" className={`choice ${people === '5+' ? 'choice--selected' : ''}`} aria-pressed={people === '5+'} onClick={() => setPeople('5+')}>Хотя бы 5</button><button type="button" className={`choice ${people === 'exact' ? 'choice--selected' : ''}`} aria-pressed={people === 'exact'} onClick={() => setPeople('exact')}>Ровно N</button></div>{people === 'exact' ? <label>Сколько человек?<Input aria-label="Сколько человек?" type="number" min="2" max="12" step="1" value={exactPeople} onChange={event => setExactPeople(event.target.value)} /></label> : null}{range === undefined ? <p className="form-error">Укажи целое число участников от 2 до 12</p> : null}</div>
