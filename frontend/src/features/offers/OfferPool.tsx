...
export function OfferPool({ offers, onSignal, onChanged, hasActiveSignal = false, providerState, onRetry }: { offers: Offer[]; onSignal: () => void; onChanged: () => void; hasActiveSignal?: boolean; providerState?: string; onRetry?: () => void }) {
  if (!offers.length) {
    ...
    const description = !hasActiveSignal ? 'Сообщи, когда свободен, — мы покажем конкретные варианты.' : unavailable ? 'Сигнал сохранён. Попробуй поиск ещё раз.' : noSource ? 'Попробуй другое время или категории.' : noFeasible ? 'События и места есть, но условия пока не совпали.' : 'Когда появится подходящий план, пригласим тебя.'
    return <section className="offer-pool offer-pool--empty"><EmptyState title={title} action={!hasActiveSignal ? { label: 'Подать сигнал', onClick: onSignal } : onRetry ? { label: 'Повторить поиск', onClick: onRetry } : undefined}>{description}</EmptyState></section>
  }
  ...
  return <section className="offer-pool" aria-labelledby="offer-pool-title">
    <SectionHeader title="Приглашения" action={<button type="button" className="text-action" onClick={onSignal}>+ Подать сигнал</button>} />
