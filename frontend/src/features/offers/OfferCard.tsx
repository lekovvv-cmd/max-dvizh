...
export function OfferCard({ offer, onChanged }: { offer: Offer; onChanged: () => void }) {
  ...
  return <article id={`offer-${offer.id}`} className={`offer-card ${offer.is_near ? 'offer-card--near' : ''}`}>
    ...
    {offer.price_kind === 'FROM' ? <p>Итоговая цена может быть выше</p> : null}
    {offer.opening_hours_unverified ? <p>Режим работы лучше проверить по ссылке источника</p> : null}
    <p className="offer-card__people">{offer.remaining_to_confirm > 0 ? `${offer.accepted_count} из ${offer.required_min_people} · нужен ещё ${offer.remaining_to_confirm}` : offer.remaining_capacity > 0 ? `${offer.accepted_count} в деле · можно присоединиться` : `${offer.accepted_count} из ${offer.effective_max} · мест нет`}</p>
