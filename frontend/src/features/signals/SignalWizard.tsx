...
export function SignalWizard({ group, locations, addLocation, onDone }: { group: Group; locations: Location[]; addLocation: () => Promise<Location>; onDone: () => void }) {
  ...
  const [start, setStart] = useState(localInput(new Date(Date.now() + 7_200_000)))
  const [end, setEnd] = useState(localInput(new Date(Date.now() + 18_000_000)))
  const [people, setPeople] = useState('3+')
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')

  const budgetValue = budget === 'custom' ? Number(customBudget) : budget === 'any' ? null : Number(budget)
  const radiusValue = radius === 'custom' ? Number(customRadius) : radius === 'any' ? null : Number(radius)
  const validStepOne = Boolean(start && end && new Date(start) < new Date(end))
  const validStepTwo = radiusValue === null || Boolean(locationId)

  async function ensureLocation() {
    if (!radiusValue || locationId) return
    const location = await addLocation()
    setLocationId(location.id)
  }

  async function submit() {
    if (!validStepOne || !validStepTwo || (budget === 'custom' && !customBudget) || (radius === 'custom' && !customRadius)) return
    setBusy(true); setError('')
    const groupSize = people === '5+' ? [5, 12] : people === 'Ровно 5' ? [5, 5] : people === '3+' ? [3, 12] : [1, 12]
    try {
      await api.signal({ group_id: group.id, city_slug: group.city_slug, activity_category: category, available_from: new Date(start).toISOString(), available_to: new Date(end).toISOString(), budget_max: budgetValue || null, origin_location_id: radiusValue ? locationId : null, radius_km: radiusValue || null, min_people: groupSize[0], max_people: groupSize[1], expires_at: new Date(Date.now() + 86_400_000).toISOString() })
      onDone()
  ...
  return <section className="wizard" aria-labelledby="signal-title">
    ...
    {step === 3 ? <>
      ...
      <div className="wizard__block"><h2>Размер компании</h2><div className="choices"><Choice selected={people === 'Неважно'} onClick={() => setPeople('Неважно')}>Неважно</Choice><Choice selected={people === '3+'} onClick={() => setPeople('3+')}>3+</Choice><Choice selected={people === '5+'} onClick={() => setPeople('5+')}>5+</Choice><Choice selected={people === 'Ровно 5'} onClick={() => setPeople('Ровно 5')}>Ровно 5</Choice></div></div>
