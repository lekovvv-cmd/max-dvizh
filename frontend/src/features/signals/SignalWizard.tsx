...
const initialWhen = (): When =>
  new Date().getHours() >= 20 ? 'tomorrow' : new Date().getHours() >= 18 ? 'after20' : 'evening'
...
export function SignalWizard({
  ...
  locations: Location[]
  activeBatch?: Intent[]
  onAddPlace: (city: string | null) => void
  onDone: () => void
}) {
  ...
  const [start, setStart] = useState(
    existing?.available_from
      ? formatLocalDateTimeInput(new Date(existing.available_from))
      : formatLocalDateTimeInput(rangeFor(initialWhen())[0]),
  )
  const [end, setEnd] = useState(
    existing?.available_to
      ? formatLocalDateTimeInput(new Date(existing.available_to))
      : formatLocalDateTimeInput(rangeFor(initialWhen())[1]),
  )
  const [selectedCategories, setSelectedCategories] = useState<string[]>(
    existing?.activity_categories || ['any'],
  )
  const [groupIds, setGroupIds] = useState<string[]>(
    activeBatch?.map((intent) => intent.group_id) || [group.id],
  )
  const [conditionsOpen, setConditionsOpen] = useState(
    Boolean(
      existing &&
        (existing.budget_max !== null ||
          existing.radius_km !== null ||
          existing.max_people !== null ||
          existing.min_people > 2),
    ),
  )
  ...
  const selectedLocationId = places.some((place) => place.id === locationId)
    ? locationId
    : places.find((place) => place.is_default)?.id || places[0]?.id || ''
    ...
    setSelectedCategories((current) =>
      value === 'any'
        ...
        : current.includes(value)
          ? current.filter((item) => item !== value).length
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (
      ...
      peopleRange === undefined ||
      (radiusValue !== null && !selectedLocationId)
    )
      return
    ...
    if (
      ...
      setError('Укажи время, которое ещё не прошло')
      return
    }
    setBusy(true)
    setError('')
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : 'Не удалось подать сигнал')
    } finally {
      setBusy(false)
    }
  }
  return (
    <section className="wizard" aria-labelledby="signal-title">
      ...
      <form onSubmit={(event) => void submit(event)}>
        <div className="form-section">
          ...
          <div className="choices">
            <Choice
              selected={when === 'evening'}
              disabled={new Date().getHours() >= 18}
              onClick={() => chooseWhen('evening')}
            >
              Сегодня вечером
            </Choice>
            <Choice
              selected={when === 'after20'}
              disabled={new Date().getHours() >= 20}
              onClick={() => chooseWhen('after20')}
            >
              Сегодня после 20:00
            </Choice>
            <Choice selected={when === 'tomorrow'} onClick={() => chooseWhen('tomorrow')}>
              Завтра вечером
            </Choice>
            <Choice selected={when === 'weekend'} onClick={() => chooseWhen('weekend')}>
              На выходных
            </Choice>
            <Choice selected={when === 'custom'} onClick={() => chooseWhen('custom')}>
              Выбрать время
            </Choice>
          </div>
          {when === 'custom' ? (
        ...
        <div className="form-section">
          ...
          <div className="choices">
            <Choice
              selected={selectedCategories.includes('any')}
              onClick={() => toggleCategory('any')}
            >
              Всё равно
            </Choice>
              ...
              return (
                ...
                >
                  <span>
                    ...
                    <small>{formatPeople(item.member_count)}</small>
                  </span>
                  <span className="company-option__check">
        ...
        >
          <summary>
            Условия <span>необязательно</span>
          </summary>
          <div className="conditions__content">
            ...
            {places.length ? (
              <>
                ...
                <p className="form-hint">≈ расстояние по прямой, не время в пути</p>
              </>
            ) : (
              <p>
                Добавь место в выбранном городе, чтобы ограничивать расстояние.{' '}
                <button
                  type="button"
                  className="back-link"
                  onClick={() => onAddPlace(selectedCity || null)}
                >
                  Добавить место
                </button>
              </p>
            ...
            <div className="choices">
              <Choice selected={people === 'any'} onClick={() => setPeople('any')}>
                Неважно
              </Choice>
              <Choice selected={people === '3+'} onClick={() => setPeople('3+')}>
                Хотя бы 3
              </Choice>
              <Choice selected={people === '5+'} onClick={() => setPeople('5+')}>
                Хотя бы 5
              </Choice>
              <Choice selected={people === 'exact'} onClick={() => setPeople('exact')}>
                Ровно N
              </Choice>
            </div>
            {people === 'exact' ? (
        ...
        {budgetValue === undefined || radiusValue === undefined ? (
          <p className="form-error">Проверь условия</p>
        ) : null}
        {peopleRange === undefined ? (
        ...
        {radiusValue !== null && radiusValue !== undefined && !selectedLocationId ? (
          <p className="form-error">Для расстояния выбери место в городе выбранных компаний</p>
        ) : null}
        {error ? (
        ...
        <div className="wizard__footer">
          ...
          >
            {existing ? 'Сохранить' : 'Подать сигнал'}
          </Button>
        </div>
      </form>
    </section>
