...
const items: Array<{ id: Exclude<Screen, 'signal'>; icon: 'home' | 'repeat' | 'calendar' | 'users'; label: string }> = [
...
export function AppShell({ screen, name, children, onNavigate }: { screen: Screen; name: string; children: React.ReactNode; onNavigate: (screen: Screen) => void }) {
  return <main className="app-shell">
    <header className="topbar">
      <button className="brand" onClick={() => onNavigate('home')} aria-label="На главную ДВИЖ"><span><Icon name="bolt" size={17} /></span> ДВИЖ</button>
