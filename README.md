...
Migration `20260916_0003` copies each existing CandidatePlan's source facts into `candidate_plan_source_snapshots` and removes the old provider catalogue tables. Migration `20260916_0004` makes budget/origin/radius and member distance nullable for optional constraints. Revisions `20260918_0006` through `0009` add Signal batches, provider and invitation states, place metadata and Company timezone. CI checks both a fresh PostgreSQL upgrade to head and an upgrade from populated `20260916_0005`, including data preservation and category backfill. For disposable local/demo data, reset explicitly with `docker compose down -v`, then run `docker compose up --build`; never use this procedure against a deployment database.
