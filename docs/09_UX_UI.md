...
Offer cards show accepted count, remaining people/capacity and explicit Near action. Confirmed plans can remain open to joins. A collecting responder sees the plan on Home and can withdraw before cutoff; when the plan confirms, the confirmation card appears on Home immediately. Exact-N overflow is shown privately as a waitlist response. Place plans without verified opening hours state `Режим работы лучше проверить` and link to the source. A FROM price states that final price may be higher. Production does not mount development tools.
