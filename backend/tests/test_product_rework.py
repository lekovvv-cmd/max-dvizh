...
def seed(
    ...
    users = [
        User(id=f"user-{index}", max_user_id=f"max-{index}", display_name=f"User {index}")
    ...
    for index, user in enumerate(users):
        ...
        session.add(
            Intent(
                ...
                available_to=current + timedelta(hours=6),
                min_people=minimums[index],
                max_people=maximum,
            )
        )
    session.commit()
    item = NormalizedLeisureItem(
        ...
        ends_at=current + timedelta(hours=4),
        latitude=None,
        longitude=None,
        price_text=None,
        price_min=None,
    ...
    with Session(engine) as session:
        ...
        for user in users[:3]:
            product.accept_offer(
                offers[user.id].id, OfferAction(), session, SimpleNamespace(id=user.id)
            )
...
def test_first_eligible_member_gets_offer_before_minimum_signals_exist() -> None:
    ...
    with Session(engine) as session:
        users = [
            User(id=f"early-{index}", max_user_id=f"early-{index}", display_name=f"Early {index}")
            for index in range(5)
        ]
        group = Group(
            id="early-group", name="Friends", default_city_slug="ekb", created_by=users[0].id
        )
        session.add_all([*users, group])
        session.flush()
...
def test_exact_five_waitlist_promotes_first_responder_after_cancellation() -> None:
    ...
    with Session(engine) as session:
        ...
        product.cancel_accepted_offer(
            offers[users[0].id].id, session, SimpleNamespace(id=users[0].id)
        )
...
def test_mixed_minimums_preserve_confirmed_core_and_promote_conditional_member() -> None:
    ...
    with Session(engine) as session:
        ...
        assert plan.status == "CONFIRMED_OPEN"
        product.accept_offer(
            offers[users[2].id].id, OfferAction(), session, SimpleNamespace(id=users[2].id)
        )
        assert plan.status == "CONFIRMED_OPEN"
        assert [offers[user.id].status for user in users[:3]] == [
            "ACCEPTED",
            "ACCEPTED",
            "WAITING_CONDITION",
        ]
        ...
        assert (
            conditional.participant_count,
            conditional.conditional_count,
            conditional.personal_response_count,
            conditional.personal_required_min,
        ) == (2, 1, 3, 5)
        assert conditional.share_text == ""
        pending = product.offer_out(session, offers[users[3].id])
        assert (pending.accepted_count, pending.conditional_count, pending.required_min_people) == (
            2,
            1,
            2,
        )
        product.accept_offer(
            offers[users[3].id].id, OfferAction(), session, SimpleNamespace(id=users[3].id)
        )
        assert plan.status == "CONFIRMED_OPEN"
        assert offers[users[2].id].status == "WAITING_CONDITION"
        product.accept_offer(
            offers[users[4].id].id, OfferAction(), session, SimpleNamespace(id=users[4].id)
        )
        assert plan.status == "CONFIRMED"
        assert all(offer.status == "ACCEPTED" for offer in offers.values())
        product.cancel_accepted_offer(
            offers[users[4].id].id, session, SimpleNamespace(id=users[4].id)
        )
        assert plan.status == "CONFIRMED_OPEN"
        assert [offers[user.id].status for user in users[:4]] == [
            "ACCEPTED",
            "ACCEPTED",
            "WAITING_CONDITION",
            "ACCEPTED",
...
def test_place_chooses_slot_with_more_feasible_people() -> None:
    ...
    with Session(engine) as session:
        ...
        place = replace(
            ...
            source_url="https://kudago.com/place/example/",
        )
...
def test_saved_places_are_private_city_scoped_and_support_default_rename_delete() -> None:
    ...
    with Session(engine) as session:
        ...
        with pytest.raises(HTTPException) as wrong_city:
            product.create_location(
                LocationIn(label="Дом", city_slug="msk", latitude=55.7, longitude=37.6),
                session,
                owner,
            )
        assert wrong_city.value.status_code == 422
        first = product.create_location(
            LocationIn(
                label="Дом", city_slug="ekb", latitude=56.8, longitude=60.6, address_text="Улица 1"
            ),
            session,
            owner,
        )
        second = product.create_location(
            LocationIn(label="Работа", city_slug="ekb", latitude=56.9, longitude=60.7),
            session,
            owner,
        )
        ...
        assert [place.is_default for place in product.list_locations(session, owner)] == [
            True,
            False,
        ]
        session.add(
...
def test_company_city_rejects_unsupported_provider_city(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with Session(engine) as session:
        ...
        session.add_all(
            [owner, group, GroupMember(group_id=group.id, user_id=owner.id, role="OWNER")]
        )
        session.commit()
        monkeypatch.setattr(
            product,
            "supported_cities",
            lambda: {
                "ekb": {"slug": "ekb", "name": "Екатеринбург", "timezone": "Asia/Yekaterinburg"}
            },
        )
...
def test_company_city_change_cancels_mutable_state_and_preserves_confirmed_core(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = create_engine("sqlite://")
    Base.metadata.create_all(engine)
    with Session(engine) as session:
        ...
        confirmed = CandidatePlan(
            group_id=group.id,
            city_slug="ekb",
            starts_at=current + timedelta(days=1),
            ends_at=current + timedelta(days=1, hours=2),
            required_min_people=2,
            required_max_people=2,
        ...
        session.flush()
        collecting_pending = Offer(
            candidate_plan_id=collecting.id,
            user_id=owner.id,
            status="PENDING",
            expires_at=collecting.expires_at,
        )
        collecting_accepted = Offer(
            candidate_plan_id=collecting.id,
            user_id=friend.id,
            status="ACCEPTED",
            expires_at=collecting.expires_at,
        )
        confirmed_accepted = Offer(
            candidate_plan_id=confirmed.id,
            user_id=owner.id,
            status="ACCEPTED",
            expires_at=confirmed.expires_at,
        )
        confirmed_pending = Offer(
            candidate_plan_id=confirmed.id,
            user_id=friend.id,
            status="PENDING",
            expires_at=confirmed.expires_at,
        ...
        session.add_all(
            [collecting_pending, collecting_accepted, confirmed_accepted, confirmed_pending]
        )
        session.flush()
        stale_notification = OutboxNotification(
            kind="OFFER",
            user_id=owner.id,
            payload={"offer_id": collecting_pending.id},
            status="PENDING",
        )
        ...
        monkeypatch.setattr(
            product,
            "supported_cities",
            lambda: {"msk": {"slug": "msk", "name": "Москва", "timezone": "Europe/Moscow"}},
        )

        result = product.update_group_city(
            group.id, GroupCityUpdateIn(city_slug="msk"), session, owner
        )

        intents = list(session.scalars(select(Intent).order_by(Intent.type)))
        assert {intent.type: intent.status for intent in intents} == {
            "ONE_TIME": "CANCELLED",
            "RECURRING": "PAUSED",
        }
        assert collecting.status == "CANCELLED"
        assert (collecting_pending.status, collecting_accepted.status) == (
            "INVALIDATED",
            "INVALIDATED",
        )
        ...
        assert (
            result.cancelled_signals,
            result.paused_autosignals,
            result.cancelled_plans,
            result.invalidated_offers,
        ) == (1, 1, 1, 3)
...
def test_collecting_response_keeps_overlapping_offer_invalidated() -> None:
    ...
    with Session(engine) as session:
        ...
        plans = regenerate_group(
            session, group.id, "ekb", [item, replace(item, provider_id="other", title="Другое")]
        )
        selected = session.scalar(
            select(Offer).where(
                Offer.candidate_plan_id == plans[0].id, Offer.user_id == users[0].id
            )
        )
        competing = session.scalar(
            select(Offer).where(
                Offer.candidate_plan_id == plans[1].id, Offer.user_id == users[0].id
            )
        )
...
def test_signal_batch_rejects_mixed_cities_without_partial_rows(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = create_engine("sqlite://")
    Base.metadata.create_all(engine)
    with Session(engine) as session:
        ...
        session.add_all(
            [
                ...
                first,
                second,
                GroupMember(group_id=first.id, user_id=user.id),
                GroupMember(group_id=second.id, user_id=user.id),
            ]
        )
        session.commit()
        current = datetime.now(UTC)
...
def test_signal_batch_rolls_back_failed_recompute_and_refresh_repairs_idempotently(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = create_engine("sqlite://")
    Base.metadata.create_all(engine)
...
def test_scheduler_uses_active_recurring_rules_without_visiting_app(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = create_engine("sqlite://")
    Base.metadata.create_all(engine)
    with Session(engine) as session:
        ...
        refreshed = []
        monkeypatch.setattr(
            "app.modules.matching.scheduler.fetch_items",
            lambda query: (queries.append(query), ProviderResult([], False, datetime.now(UTC)))[1],
        )
        monkeypatch.setattr(
            "app.modules.matching.scheduler.regenerate_group",
            lambda _session, group_id, city, items: refreshed.append((group_id, city, items)),
        )
...
def test_auto_signal_save_returns_before_provider_evaluation(
    ...
    with Session(engine) as session:
        ...
        session.commit()
        monkeypatch.setattr(
            product, "fetch_items", lambda *_: pytest.fail("provider must run after response")
        )
...
def test_signal_batch_persists_provider_outage_and_owner_can_retry(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = create_engine("sqlite://")
    Base.metadata.create_all(engine)
    with Session(engine) as session:
        ...
        group = Group(
            id="signal-group", name="Friends", default_city_slug="ekb", created_by=user.id
        )
        session.add_all([user, group, GroupMember(group_id=group.id, user_id=user.id)])
        session.commit()
        current = datetime.now(UTC)
        ...
        monkeypatch.setattr(
            product, "fetch_items", lambda *_: ProviderResult([], False, current, unavailable=True)
        )
