    ...
    with Session(engine) as session:
        ...
        for index in range(5):
            ...
            intent = Intent(
                ...
                available_from=now,
                available_to=now + timedelta(hours=4),
                budget_max=500,
            ...
            plan = CandidatePlan(
                ...
                required_min_people=1,
                required_max_people=6,
                status="COLLECTING",
                expires_at=now + timedelta(hours=1),
            )
            session.add_all(
                [group, location, intent, plan, GroupMember(group_id=group.id, user_id=user.id)]
            )
            session.add(
                CandidatePlanSourceSnapshot(
                    ...
                    latitude=56.8,
                    longitude=60.6,
                    price_text=None if index == 4 else "400 ₽",
                    parsed_price=400,
                    source_url=None,
                    image_url=None,
            ...
            session.add(
                CandidatePlanMember(
                    ...
                    user_id=user.id,
                    intent_id=intent.id,
                    compatibility="NEAR" if index == 4 else "EXACT",
                    distance_km=1.2,
                    budget_delta=50 if index == 4 else None,
...
def test_creating_candidate_plan_persists_one_source_snapshot_only() -> None:
    ...
    with Session(engine) as session:
        ...
        session.commit()
        provider_item = NormalizedLeisureItem(
            provider="KUDAGO",
            provider_id="one",
            item_type="EVENT",
            city_slug="ekb",
